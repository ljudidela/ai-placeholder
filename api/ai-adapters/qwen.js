export class QwenAdapter {
  constructor() {
    this.name = "qwen";
    this.modelUri = `gpt://${process.env.YANDEX_FOLDER_ID}/yandexgpt/latest`;
  }
  ...
  async generateCode(prompt) {
    ...
    const requestBody = {
      ...
      messages: [
        {
          role: "system",
          content: `Ты — инженер с 15-летним стажем. Ты НЕНАВИДИШЬ, когда проект не запускается после npm install && npm run dev.
            Ты знаешь, что заказчик будет материться, если:
            - index.html в public/
            - нет package.json
            - нет скриптов
            - нет README
            ...
            Поэтому ты ВСЕГДА проверяешь чек-лист выше и никогда не пропускаешь эти вещи.

            Если ты нарушишь хоть один пункт — тебя уволят.
            Отвечай ТОЛЬКО чистым JSON по схеме. Никаких извинений, никакого markdown, никаких пояснений.`,
        },
